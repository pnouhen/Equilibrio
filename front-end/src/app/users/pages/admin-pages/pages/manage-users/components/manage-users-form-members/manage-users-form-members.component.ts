...
import { SelectedCityUsersService } from './../../services/SelectedCityUsers.service';
...
import { SelectedCategoryService } from '../../services/SelectedCategory.service';
import { SelectedGradeService } from '../../services/SelectedGrade.service';
...
export class ManageUsersFormMembersComponent implements OnInit {
  ...
  constructor(
    ...
    public selectedGradeService: SelectedGradeService,
    public manageUsersService: ManageUsersService,
    public citiesService: CitiesService,
    public selectedCityUsersService: SelectedCityUsersService,
  ) {}
  ...
  activeCategory(event: Event) {
    this.selectedCategoryService.toggleSelected(event);

    this.selectedGradeService.closeSelected();
    this.selectedCityUsersService.closeSelected();
  }
  ...
  toggleGrade(event: Event) {
    ...
    if (this.gradeData.length > 0) this.selectedGradeService.toggleSelected(event);
  }
    ...
    if (isSubmitValid) {
      ...
      if (this.idUdpate !== '') {
        ...
        if (memberUpdate) {
          ...
          this.formMessages[0].text = 'La mise à jour a fonctionné';
        }
      } else {
        ...
        // Managing the display of the message after submit
        this.formMessages[0].text = 'Le membre a bien été ajouté';
      }
      // Reset all
      this.memberName = '';
