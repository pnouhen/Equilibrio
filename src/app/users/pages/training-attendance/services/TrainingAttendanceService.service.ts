...
import { StudentRegularityService } from './StudentRegularityService.service';
...
export class TrainingAttendanceService {
  ...
  constructor(
    public studentRegularityService: StudentRegularityService,
  ...
  async updateUsersSchedule(newSchedule: string) {
    ...
    const updateUsers = await Promise.all(
      newUsersFilters.map(async (user) => ({
        ...user,
        numberTrainningSessions: await this.studentRegularityService.calculStudentRegularity(
