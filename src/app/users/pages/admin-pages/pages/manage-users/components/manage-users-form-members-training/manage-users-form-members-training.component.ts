...
export class ManageUsersFormMembersTrainingComponent implements OnInit {
  ...
  toggleCityUsers(event: Event) {
    ...
    if (this.blockCityChoice()) this.selectedCityUsersService.toggleSelected(event);
  }
  ...
  onSubmitSchedule() {
    ...
    if (addSchedules.length > 0) {
      ...
      if (!isCitySelectedCities) {
        ...
        this.formMessages[1].text = 'La ville est déjà enregistrée';
      }
    } else {
      this.isFormValid = false;
      this.formMessages[1].text = 'Au moins un des champs est manquant';
    }
  }
